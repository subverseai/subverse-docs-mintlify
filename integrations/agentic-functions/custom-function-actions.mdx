...
The Shopify actions below connect to a Shopify store via a saved Shopify credential. All actions support `credentialId` plus an `options` object, and can override the detected authentication method with `options.authentication`.
...
| `options.authentication` | No | `apiKey`, `accessToken`, or `oAuth2` |
