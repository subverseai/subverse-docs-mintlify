...
Upsert rows by a lookup column. For each row, if an existing row has a matching value at the lookup column index, that row is updated in place; otherwise the row is appended after the last row with data.
...
| `options.rows` | Yes | Array of rows to upsert. Each row is an array of cell values |
| `options.lookupColumnIndex` | Yes | 0-based column index used to match rows (0 = column A) |
| `options.valueInputOption` | No | `RAW` or `USER_ENTERED`. Default `USER_ENTERED` |
