...
<Info title="My Drive vs. Shared Drives">
  By default, every action below only looks at "My Drive" — the personal Drive space belonging to whoever the credential connects to. If a file instead lives in a **Shared Drive** (a Drive space owned jointly by a team, common in Google Workspace organisations), add `includeSharedDrives: true` to `options`, and optionally `driveId` to point at one specific Shared Drive. Every action accepts these two options even though they're not repeated in every table below.
</Info>
...
| `options.includeSharedDrives` | No | Also look in Shared Drives, not just "My Drive". Default `false` |
| `options.driveId` | No | Limit the lookup to one specific Shared Drive |
...
| `options.includeSharedDrives` | No | Upload into a Shared Drive instead of "My Drive". Default `false` |
| `options.driveId` | No | The specific Shared Drive to upload into |
...
Create a native Google Doc, Sheet, or Slides file directly from plain text — Google Drive converts the text into a real, editable file of that type, not a `.txt` file sitting in Drive. Use this instead of Upload File when you want the result to open as an actual Google Doc/Sheet/Slides file rather than a plain-text file.

| Parameter | Required | Description |
|---|---|---|
| `credentialId` | Yes | Google Drive credential ID (service account or OAuth2) |
| `options.name` | Yes | Name for the new file |
| `options.text` | Yes | Plain text content — Google Drive auto-converts it into the native format you pick below |
| `options.targetType` | Yes | Which native Google app format to create: `document`, `spreadsheet`, or `presentation` |
| `options.folderId` | No | Folder to create the file into — the `id` from a `listFiles`, `getFile`, or `createFolder` result. Omitted creates it at the top level of "My Drive" |
| `options.includeSharedDrives` | No | Create inside a Shared Drive instead of "My Drive". Default `false` |
| `options.driveId` | No | The specific Shared Drive to create it in |
...
| `options.includeSharedDrives` | No | Create the folder inside a Shared Drive instead of "My Drive". Default `false` |
| `options.driveId` | No | The specific Shared Drive to create it in |
